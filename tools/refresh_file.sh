...
# $1 - file name $2 full URL $3 final destination directory $4 optional check command


if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
  echo Usage: $0 file_name full_url destination_dir optional_check_command
  exit 124
fi
...
remoteetag=`curl -I $2 | grep etag | cut -f 2 -d '"'`
