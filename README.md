...
Для обновления базы с новыми файлами выполнить пункты 3, 4, 5, 8

Этот чеклист установки не актуален. Скоро image будет размещен на dockerhub (blitvin/flibusta-at-home) и все что нужно это
изменить docker_compose.yml в соответствии с Вашей средой ( директории, адреса подсетей в докере и т.п.). После запуска контейнера
идем на вкладку "Сервис", скачиваем с помощью кнопок операций DB dump, файлы обложек, последние обновления и обновляем базу данных.
...
3. `FLIBUSTA_DBHOST` имя хоста на которой установлена база данных. В докере по умолчанию совпадает с именем сервиса 
